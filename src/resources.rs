...
impl Resources {
    ...
    pub fn load_image_absolute(&self, path: PathBuf) -> Result<image::DynamicImage, Error> {
        let image = image::open(path)?;
        Ok(image)
    }

    pub fn load_image(&self, resource_name: &str) -> Result<image::DynamicImage, Error> {
        let image = image::open(resource_name_to_path(&self.root_path, resource_name))?;
        Ok(image)
    }
