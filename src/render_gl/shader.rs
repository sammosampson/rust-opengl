...
impl Program {
    pub fn from_res(gl: &gl::Gl, res: &Resources, resource_names: &[&str]) -> Result<Program, Error> {
    ...
    pub fn set_uniform_vec2f(&self, name: &str, value0: f32, value1: f32) {
        let location = self.get_uniform_location(name);
        unsafe {
            self.gl.Uniform2f(location, value0, value1);
        }
    }

    pub fn set_uniform_vec2ui(&self, name: &str, value0: u32, value1: u32) {
        let location = self.get_uniform_location(name);
        unsafe {
            self.gl.Uniform2ui(location, value0, value1);
        }
    }

    pub fn set_uniform_vec3f(&self, name: &str, value0: f32, value1: f32, value2: f32) {
        let location = self.get_uniform_location(name);
        unsafe {
            self.gl.Uniform3f(location, value0, value1, value2);
        }
    }
...
impl Shader {
    pub fn from_res(gl: &gl::Gl, res: &Resources, name: &str) -> Result<Shader, Error> {
        const POSSIBLE_EXT: [(&str, gl::types::GLenum); 3] =
            [(".vert", gl::VERTEX_SHADER), (".geom", gl::GEOMETRY_SHADER), (".frag", gl::FRAGMENT_SHADER)];
