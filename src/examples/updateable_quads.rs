...
pub fn run() -> Result<(), failure::Error> {
    ...
    window_manager.loop_until_window_quit(|total_time,__| {
        let total_time_enlarged = total_time * 8.0;
        println!("{}", total_time);
        let points: Vec<QuadShapeVertex> = vec![
            QuadShapeVertex::with_position_and_size((900.0, 700.0).into(), (300.0 + total_time_enlarged, 300.0 + total_time_enlarged).into(), (0.0, 0.0, 0.0).into())
        ];
        colour_buffer.set_used(&gl);
        colour_buffer.clear(&gl);
        blender.set_used(&gl);
        renderer.update_vertices(points, 4);
        renderer.render(&gl);
        blender.set_unused(&gl);
        window.swap();
    })?;
