...
pub fn run() -> Result<(), failure::Error> {
    ...
    let window = window_builder
        .with_title("Vertex coloured quad")
        .with_width(900)
        .with_height(700)
        .build()?;
...
pub fn quad(res: &resources::Resources, gl: &gl::Gl)
    ...
    let shader_program = render_gl::Program
        ::from_res(&gl, &res, &["shaders/first/coloured_primitive.vert", "shaders/first/coloured_primitive.frag"])?;

    let vertices: Vec<render_gl::vertices::VertexColouredVertex> = vec![
        render_gl::vertices::VertexColouredVertex::with_position_and_colour((1400.0, 1400.0, 0.0).into(), (1.0, 1.0, 1.0, 0.0).into()),
        render_gl::vertices::VertexColouredVertex::with_position_and_colour((1400.0, 0.0, 0.0).into(), (1.0, 1.0, 1.0, 0.0).into()),
        render_gl::vertices::VertexColouredVertex::with_position_and_colour((0.0, 1400.0, 0.0).into(), (0.0, 1.0, 0.0, 0.0).into()),
        render_gl::vertices::VertexColouredVertex::with_position_and_colour((0.0, 0.0, 0.0).into(), (1.0, 0.0, 0.0, 0.0).into()),
    ];

    render_gl::rendering::VertexColouredVertexPrimitiveRenderer::new(gl, shader_program, vertices, 4)
}